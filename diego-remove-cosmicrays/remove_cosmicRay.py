...
for archivo in archivos:
    ...
    mask, clean_image = astroscrappy.detect_cosmics(
        image, sigclip=5.0, sigfrac=0.3, objlim=5.0, cleantype="median", verbose=False
    )
    ...
    hdulist.writeto(
        os.path.join(ruta, archivo.replace(".fits", "_c.fits")), overwrite=True
    )
    hdulist[0].data = residual_image
    hdulist.writeto(
        os.path.join(ruta, archivo.replace(".fits", "_res.fits")), overwrite=True
    )
