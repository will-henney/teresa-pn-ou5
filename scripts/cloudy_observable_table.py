...
def _parse_model_name(p: Path) -> Dict[str, Any]:
    ...
    toks = stem.split("-")
    out = {"model": stem, "EOS": None, "Z_amp": 1, "Z_lambda": "homog"}
    if len(toks) >= 4:
        eos = toks[3].lower()
...
def main(
    ...
    pattern: str = typer.Option(
        "ou5a-T115-L660-n-lha*-global.yaml",
        "--pattern",
        "-p",
        help="Glob pattern (no regex) to find YAMLs",
    ),
    out_csv: Path = typer.Option(
        Path("ou5a-observables.csv"), "--out", "-o", help="Output CSV path"
    ),
):
    ...
    for yml in files:
        ...
        o3_4363_total = (
            sum(v for v in (o3_4363_co, o3_4363_rr, o3_4363_cx) if v is not None)
            if any(v is not None for v in (o3_4363_co, o3_4363_rr, o3_4363_cx))
            else None
        )
        ...
        r4363_5007 = (
            (o3_4363_total / o3_5007)
            if (o3_4363_total is not None and o3_5007 and o3_5007 > 0)
            else None
        )
